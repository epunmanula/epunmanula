...
out_path_1 = os.path.join(dir_path, "epun-manula-floating-banner.svg")
out_path_2 = r"c:\em\web\reeme\epun-manula-floating-banner.svg"
...
svg_template = f'''<svg width="1200" height="340" viewBox="0 0 1200 340" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  <defs>
    ...
    <style>
      ...
      /* Smooth Organic Floating Animations for Tech Stack Icons (Ehata Mehata Wena Animations) */
      @keyframes floatMovement1 {{
        0%, 100% {{ transform: translate(0px, 0px); }}
        50% {{ transform: translate(-9px, -5px); }}
      }}
      @keyframes floatMovement2 {{
        0%, 100% {{ transform: translate(0px, 0px); }}
        50% {{ transform: translate(9px, 6px); }}
      }}
      @keyframes floatMovement3 {{
        0%, 100% {{ transform: translate(0px, 0px); }}
        50% {{ transform: translate(-8px, 6px); }}
      }}
      @keyframes floatMovement4 {{
        0%, 100% {{ transform: translate(0px, 0px); }}
        50% {{ transform: translate(8px, -6px); }}
      }}
      @keyframes floatMovement5 {{
        0%, 100% {{ transform: translate(0px, 0px); }}
        50% {{ transform: translate(-7px, -6px); }}
      }}
      @keyframes floatMovement6 {{
        0%, 100% {{ transform: translate(0px, 0px); }}
        50% {{ transform: translate(9px, -4px); }}
      }}

      .icon-float-1 {{ animation: floatMovement1 3.5s ease-in-out infinite; }}
      .icon-float-2 {{ animation: floatMovement2 4.2s ease-in-out infinite; }}
      .icon-float-3 {{ animation: floatMovement3 3.8s ease-in-out infinite; }}
      .icon-float-4 {{ animation: floatMovement4 4.5s ease-in-out infinite; }}
      .icon-float-5 {{ animation: floatMovement5 4.0s ease-in-out infinite; }}
      .icon-float-6 {{ animation: floatMovement6 3.7s ease-in-out infinite; }}
  ...
  <!-- LEFT SIDE TECH STACK ICONS (Compact Grid with Floating Animations) -->
  <!-- 1. REACT LOGO -->
  ...
  <!-- CENTER MAIN CONTENT -->
  <!-- 1. EMBEDDED ANIMATED 3D ROTATING LOGO -->
  ...
  <!-- RIGHT SIDE TECH STACK ICONS (Compact Grid with Floating Animations) -->
  <!-- 7. WEBASSEMBLY LOGO -->
...
print("Floating Tech Stack Animated SVG banner generated successfully!")
